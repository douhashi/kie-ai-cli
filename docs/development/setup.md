...
シェル補完が読む索引 `internal/catalog/index.tsv`（`id / category / vendor` を
タブ区切りで 1 モデル 1 行）も同じ実行が書く。カタログからの導出物なので
**手で編集しない**。コミット済みの両者が食い違っていないことは `mise run test` が
byte 比較で落とす。索引だけが消えた場合は、カタログが変わらなくても書き直す
（日付はカタログを説明するものなので動かさない）。

これら 3 ファイルはバイナリへ焼き込まれ、うち `catalog.json` と
`generated_at.txt` の 2 件が Release 資産として公開される（`catalog update` の
取得先）。索引は公開せず、取得側がカタログから導出する。生成日から 90 日
（`catalog.MaxAge`）を過ぎたカタログは、焼き込み・手元のどちらであっても
その旨を警告する。
...
同じタスクが `dist/checksums.txt`（名前は `dist/` からの相対）まで書く。環境変数
`KIE_AI_VERSION` を渡すと版を ldflags で焼き込み、5 つすべてに入っていることと、
手元向けの 1 つの `--version` がその版を返すことを確認する。**リリースはこのタスクを
呼ぶだけ**なので、公開の手順は PR の CI が毎回通る（渡さない場合は従来どおり
Go ツールチェーンが記録した版を返す）。
...
索引が同じカタログからの導出と byte 一致することも、ここで落とす。

補完スクリプトの構文検査（`bash -n` / `zsh -n`）も `mise run test` に入る。
当該シェルが無い環境では skip する。**時間の検査は置かない**。実測は
...
CLI バイナリの配布はもう 1 つのワークフローが持つ。起点が定期実行ではなく人である
ことだけが、上の 2 つとの違いである。

| ワークフロー | 内容 |
|---|---|
| `release` | tag `v*` の push で `mise run test` と `mise run build-all` を通し、バイナリ 5 件と `checksums.txt` を tag の Release へ上げる |

失敗を起票しないのはここだけである。**タグを打った本人がその場にいる**ので、
気づく経路が run 履歴の目視で足りる。`--latest` を明示するのは、tag `catalog` の
Release と `/releases/latest` を取り合わせないためで、README の入手手順は
`/releases/latest/download/` を叩く。
